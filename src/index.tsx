import { List } from "@raycast/api";
import { useState } from "react";
import { getBridgeIpAddress } from "./hue";
import { useCachedState } from "@raycast/utils";

export default function Command() {
  const [bridgeIpAddress, setBridgeIpAddress] = useCachedState<string | undefined>("bridgeIpAddress", undefined);
  const [isLoading, setIsLoading] = useState(bridgeIpAddress === undefined);

  if (bridgeIpAddress === undefined) {
    getBridgeIpAddress().then((bridgeIpAddress) => {
      setIsLoading(false);
      setBridgeIpAddress(bridgeIpAddress);
    });
  }

  return <List isLoading={isLoading}>{bridgeIpAddress && <List.Item title={bridgeIpAddress} />}</List>;
}
